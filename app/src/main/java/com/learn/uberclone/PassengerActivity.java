...
public class PassengerActivity extends FragmentActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    private Button btnPassenger_RequestCar, btnPassenger_LogOutFromPassengerActivity, btnPassenger_BeepBeep;
    ...
    private void assignUI() {
        ...
        btnPassenger_BeepBeep = findViewById(R.id.btnPassenger_BeepBeep);
    }
    ...
    private void callAllOnClickEvent() {
        ...
        btnPassenger_BeepBeep.setOnClickListener(this);
    }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btnPassenger_LogOutFromPassengerActivity) {
            onClick_LogOutUser();
        } else if (view.getId() == R.id.btnPassenger_BeepBeep){
            getDriverUpdates();
        } else {
            if (isUberCancelled) {
                onClick_RequestUber();
                                            ...
                                            if (drivers.size() > 0 && e == null) {
                                                ...
                                                for (ParseUser driverOfRequest : drivers) {
                                                    ...
                                                    if (ContextCompat.checkSelfPermission(PassengerActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                                                        ...
                                                        // Log.i("Miles", "Miles : " + milesDistance);

                                                        if (milesDistance <= 0.7) {
                                                            // Log.i("Miles", "jalan yang ke -1");
                                                            requestObject.deleteInBackground(new DeleteCallback() {
                                                                @Override
                                                                public void done(ParseException e) {
                                                                    if (e == null) {
                                                                        Toast.makeText(PassengerActivity.this,
                                                                                "Your Uber is ready! Hurray",
                                                                                Toast.LENGTH_SHORT)
                                                                                .show();
                                                                        isCarReady = false;
                                                                        isUberCancelled = true;
                                                                        btnPassenger_RequestCar.setText("You can order a new Uber now!");
                                                                    }
                                                                }
                                                            });
                                                        } else {
                                                            Log.i("Miles", "jalan yang ke -2");
                                                            float roundedDistance = Math.round(milesDistance * 10) / 10;

                                                            Toast.makeText(PassengerActivity.this,
                                                                    "Hurray! " + requestObject.get("driverOfMe") + " is " + roundedDistance + " miles away from you! Please wait!",
                                                                    Toast.LENGTH_SHORT)
                                                                    .show();

                                                            // Add a marker on Driver Location and also Passenger Location then move the camera
...
//                Log.i("Timer", "Status Car Ready Di Bawah : " + isCarReady);
